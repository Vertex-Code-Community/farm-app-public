...
export function initAutoHeight() {
    const updateOffset = () => {
        const allInvalid = document.querySelectorAll('.invalid.modified:not(:focus-within)');

        const otpErrors = document.querySelectorAll('.code-input-with-error.error');

        const filteredFields = [...allInvalid, ...otpErrors].filter(el =>
            !el.closest('.ignore-error')
        );

        const offset = filteredFields.length * 28;
