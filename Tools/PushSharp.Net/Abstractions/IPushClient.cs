...
public interface IPushClient
{
    ...
    /// Requires an <see cref="IDeviceRegistrationStore"/> to be registered via
    /// <c>builder.AddDeviceStore&lt;T&gt;()</c>.
    /// Dead tokens are automatically cleaned up from the store.
    /// </summary>
    Task<BatchPushResult> SendToUserAsync(
        string userId,
        PushNotification notification,
        CancellationToken cancellationToken = default);
    ...
    /// Dead tokens are automatically cleaned up from the store.
    /// </summary>
    Task<BatchPushResult> SendByTagAsync(
        string tag,
        PushNotification notification,
        CancellationToken cancellationToken = default);
    ...
    /// Requires an <see cref="IDeviceRegistrationStore"/> to be registered.
    /// Use with caution for large audiences.
    /// </summary>
    Task<BatchPushResult> SendToAllAsync(
        PushNotification notification,
        CancellationToken cancellationToken = default);
}
